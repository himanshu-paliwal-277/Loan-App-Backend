...
export const makeRepayment = async (req, res) => {
  try {
    ...
    if (loan.status !== 'approved') {
      return res.status(403).json({ message: 'Loan is not approved. Cannot make repayment.' });
    }
    ...
    if (amount > repayment.amount) {
      return res.status(400).json({ message: 'Amount is greater than repayment' });
    }
    ...
    const pendingRepayments = await Repayment.find({ loan: repayment.loan, status: 'pending' });
    if (pendingRepayments.length === 0) {
      loan.status = 'paid'; // Update loan status to paid if all repayments are complete
      await loan.save();
    }
